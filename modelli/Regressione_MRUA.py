...
k_folds=5
errore_folds=Processore.cross_validation(K=k_folds, features=NeuralNet.features, labels=NeuralNet.targets)
...
for i, ax in enumerate(asse.flatten()):
    ax.plot(np.arange(0, NeuralNet.epoche, 1), errore_folds[i], color="red")
...
plt.bar(x=np.arange(0, len(errore_folds), 1), height=[np.mean(i) for i in errore_folds], color="darkslateblue", label="Errore per fold")
